...
SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId,
                ...
                FROM Video v 
                     JOIN UserProfile up ON v.UserProfileId = up.Id
WHERE v.DateCreated >= CONVERT(DateTime, '@date')";
                    if (sortDescending)
                    {
                        sql += "ORDER BY v.DateCreated DESC";
                    }
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Title, v.Description, v.Url, v.DateCreated as VideoDate, v.UserProfileId,
                            u.[Name] as UserProfileName, u.Email, u.ImageUrl, u.DateCreated as UserProfileDate
                            FROM Video v
                            JOIN UserProfile u ON v.UserProfileId = u.Id
                           WHERE v.Id = @Id";
            ...
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            if (video.Id != DbUtils.GetInt(reader, "VideoId"))
                            {
                                video.Id = DbUtils.GetInt(reader, "VideoId");
                                video.Title = DbUtils.GetString(reader, "Title");
                                video.Description = DbUtils.GetString(reader, "Description");
                                video.Url = DbUtils.GetString(reader, "Url");
                                video.DateCreated = DbUtils.GetDateTime(reader, "VideoDateCreated");
                                video.UserProfileId = DbUtils.GetInt(reader, "VideoUserProfileId");
